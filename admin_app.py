...
with tab1:
    ...
    with col_b:
        ...
        if st.button("📤 确认发布并生成海报", type="primary"):
            if not n_pics or not n_title:
                st.error("请输入标题并上传图片！")
            else:
                with st.spinner("正在上传图片并保存数据..."):
                    poster = create_poster(n_pics, n_title)
                    if poster:
                        buf = io.BytesIO()
                        poster.save(buf, format='JPEG')
                        # 上传到 Cloudinary 并获取 URL
                        upload_result = cloudinary.uploader.upload(buf.getvalue())
                        ...
                        st.success("🎉 房源已发布！")
                        st.session_state.new_ai_desc = "" # 重置 AI 描述
                        st.rerun()
...
with tab2:
    ...
    try:
        ...
        if not df.empty:
            ...
            # 4. 详细修改表单
            with st.expander("📝 修改房源详细内容", expanded=True):
